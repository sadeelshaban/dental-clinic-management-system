...
- **Global error handling:** `Middleware/ExceptionHandlingMiddleware.cs` (business rules → 400; missing JWT claims → 401). Registered after login rate-limit middleware in `Program.cs`.
...
│       ├── Controllers/          # 18 controllers (Auth, Health, Patients, Users, Doctors, Appointments, Visits, TreatmentCategories, Treatments, PatientTreatments, Payments, PaymentMethods, ExpenseCategories, Suppliers, Expenses, ExpensePayments, Attachments, Reports)
│       ├── Data/                 # DentalClinicDbContext (scaffolded), DatabaseSeeder
│       ├── DTOs/                 # Request/response types per module
│       ├── Models/               # Scaffolded entities + keyless DB views
...
│       ├── Services/             # Interfaces/ + Implementations/ (all domain services)
│       ├── Common/               # BusinessRuleException, AttachmentFileValidator
...
| GET | `/api/patients/{patientId}/financial` | ADMIN, DOCTOR, SECRETARY | Patient financial statement: totals (from `patient_financial_summary` view for non-DOCTOR; DOCTOR header totals summed from own lines), per-treatment lines (from `patient_treatment_financials` view), payment history. DOCTOR actors see only their own lines/payments | — → `PatientFinancialStatementDto` |
| GET/POST/PUT | `/api/expensecategories`, `/api/suppliers`, `/api/expenses`, `/api/expensepayments` | Mixed (see Swagger) | Expense categories, suppliers, expense obligations, expense payments (+ void). Expense void: `POST /api/expenses/{expenseId}/void` with required reason (blocked if valid payments exist). Supplier statement: `GET /api/suppliers/{supplierId}/statement` |
| POST upload, GET `{id}/download`, GET list, DELETE | `/api/attachments` | Upload: ADMIN/SECRETARY; download/list: clinical staff; delete: ADMIN/SECRETARY (+ uploader rule in service) | Files stored on disk under configurable `Uploads:Path`; **no public static `/uploads/` URL** — download requires JWT + clinic match. Max 10 MB; PDF/images validated with magic bytes in service layer |
| GET | `/api/reports/daily`, `/monthly`, `/comparison`, `/patient-directory`, `/outstanding-balances` | Financial (`daily`/`monthly`/`comparison`): ADMIN only; directory/outstanding: clinical staff | Dashboard metrics from DB views. `daily` returns `{ outstandingPatientBalances, items[] }` (per-day revenue/expense/profit in `items`). Monthly/comparison DTOs include `outstandingPatientBalances` (clinic-wide current snapshot from `patient_financial_summary`) |

Business rules in API: ALL queries filtered by JWT `clinic_id` (never client input); patient search matches patient number, names, phone, email, national ID; gender normalized; emails lowercased. **Patient CREATE/UPDATE/DEACTIVATE are audited.** Users: roles restricted to ADMIN/DOCTOR/SECRETARY; email unique globally (application rule) and per clinic (DB index); DOCTOR creation/promotion creates the linked doctor profile in the same transaction; role changes away from DOCTOR are rejected when clinical history exists; self-role-change, self-deactivation, and last-active-admin removal are rejected (400 via `BusinessRuleException`). Deactivated users fail login (401) and `/api/auth/me` returns 404; inactive users cannot change password. Password/hash/token fields never appear in any response. Appointments: full scheduling validation per §5 Phase 2 patterns (time ordering, working hours, closed days, slot grid, overlap with blocking statuses, transactional doctor-row locking); invalid status transitions rejected (400); COMPLETED/CANCELLED/NO_SHOW are terminal and cannot be modified. Clinical records (§5 Phase 3): historical price/name snapshots immutable; `finalAmount` DB-generated; discount ≤ qty×price; visit links must match clinic+patient; inactive patients/catalog items rejected for new records; SECRETARY read-only on clinical modules; no destructive deletes anywhere. Billing (§5 Phase 4): revenue = valid money received only; status server-derived after create/void; overpayments rejected; void keeps records stored but excluded from all totals; treatment row reloaded after FOR UPDATE lock before total/status recompute; statement uses existing DB views as source of truth. Expenses (§5 Phase 5): mirror billing on payable side; expense-level and payment-level voids with required reason.

**Complete route list:** use Swagger UI at `http://localhost:5062/` in Development (always current).
...
| 3 | Patient number generation race condition (`GeneratePatientNumberAsync` read-last-then-increment) | Medium | Concurrent creates can violate `uq_patient_number` | Short retry loop on duplicate key in `PatientService` (3 attempts); transactional MAX+1 remains future improvement | OPEN (mitigated) |
| 4 | Appointment overlap not enforceable in MySQL | Medium | No exclusion constraints in MariaDB | Enforce per-doctor overlap check in service layer within a transaction | RESOLVED (service-layer enforcement + smoke-verified) |
| 5 | Audit coverage incomplete: patients not audited initially | Low | Patient module predated audit wiring | **RESOLVED (2026-08-23):** `PatientService` audits CREATE/UPDATE/DEACTIVATE; other modules audited per Phases 1–6 | **RESOLVED** |
| 6 | `attachments` table exists but no upload endpoint/storage | Low | Module not started | Phase 6 complete; authenticated download (no public static serving) | **RESOLVED** |
...
| 11 | README endpoint list incomplete/outdated | Low | Docs lag | Root + backend README synced 2026-08-23; Swagger is canonical endpoint catalog | **RESOLVED** |
...
- **Build status:** `dotnet build` — **SUCCESS** (2026-08-23, net10.0, 0 errors / 0 warnings after Phases 1–7 + post-fix hardening).
- **Automated test projects:** none yet (xUnit planned for Phase 9).
- **Smoke test scripts:** PowerShell smoke suites (`scripts/phase1_smoke.ps1` … `phase6_smoke.ps1`) were used during Phases 1–6 development but are **no longer in the repository** (removed during repository cleanup). Historical results below are retained for traceability; re-validation today should use Swagger/manual calls or reintroduce tests in Phase 9.
